...
- **`src/types.ts` is the single source of truth** for the producer↔UI contract: `GraphNode` (kind `file`|`page`, layer, package, feature, routePath, + `churn?`/`tested?`), `GraphEdge` (type `import`|`navigate`|`uses`|`api`), `GraphData` (+ `insights`, `coupling`, `apiCatalog`), `InsightsReport`, `PackageCoupling`, plus `GitInsightData`/`PolicyConfig`/`InsightInputs`/`GraphDiff`/`ApiCatalog`/`ApiEndpoint`. Change types here first; everything downstream of `buildGraph` is language-agnostic. `buildGraph(scan, parse, inputs?)` and `computeInsights(graph, inputs?)` take optional `InsightInputs` (git/coverage/policy) — both stay backward-compatible when omitted.
- **API Catalog** (`src/api-catalog.ts` + `parser/models.ts` + `mock-gen.ts`) — `buildApiCatalog(scan)` discovers every Dio HTTP call (method + path, interpolations→`{param}`, `XEndpoints.method()` constants resolved) and synthesizes a **deterministic mock request/response** from the Dart request/response models (`parser/models.ts` parses freezed `@JsonKey`/`@Default` factories + plain classes + enums into a `Registry`; `mock-gen.ts` turns a type into JSON — no network, no LLM, same spirit as `preview.ts`). Attached as `GraphData.apiCatalog`; rendered by the UI's **API Catalog** modal (header `</>` button). It's "heavy" (reads all files): computed on one-shot (`--json`/`--export`/`--check`) + the LSP refine / no-lsp background enrich, **not** the fast first-paint graph. Pure-function core is `extractCatalog(contents, featureByRel)` (unit-tested).
